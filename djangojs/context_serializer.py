...
SERIALIZABLE_TYPES = (int, bytes, str, tuple, list, dict, bool, set)
...
class ContextSerializer(object):
    ...
    def process_LANGUAGE_CODE(self, language_code, data):
        ...
        language = translation.get_language_info(
            "en" if language_code == "en-us" else language_code
        )
        if (
            not settings.JS_CONTEXT
            or "LANGUAGE_NAME" in settings.JS_CONTEXT
            or (
                settings.JS_CONTEXT_EXCLUDE
        ...
        if (
            not settings.JS_CONTEXT
            or "LANGUAGE_NAME_LOCAL" in settings.JS_CONTEXT
            or (
                settings.JS_CONTEXT_EXCLUDE
