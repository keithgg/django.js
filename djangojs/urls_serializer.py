...
RE_OPT_GRP = re.compile(
    r"\(\?\:.*\)(?:\?|\*)"
)  # Pattern for recognizing optionnal group
...
def _get_urls_for_pattern(pattern, prefix="", namespace=None):
    ...
    elif (CMS_APP_RESOLVER) and (
        issubclass(pattern.__class__, AppURLResolver)
    ):  # hack for django-cms
        ...
        if pattern.urlconf_name:
            ...
            for ns in nss:
                ...
                if (
                    ...
                    and namespaces
                    and namespaces not in settings.JS_URLS_NAMESPACES
                ):
                    continue
                if (
